// Cửa ghi vé: nhận vé từ trang web, ghi thẳng vào cua-chi/so-ve.txt trên GitHub,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (noiDungCu.split(/\r?\n/).some((d) => d.trim() === dongSach)) {
      return res.status(200).json({ ok: true, trung: true, thong_bao: "Vé này đã có trong sổ rồi." });
    }
    ...
    if (!ghi.ok) {
      ...
      return res.status(502).json({
        loi: `Không ghi được vào sổ (mã ${ghi.status}).`,
        chi_tiet: t.slice(0, 200),
      });
    }
